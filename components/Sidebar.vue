	...
	<div class="bg-primary px-5 py-4 rounded-2xl text-slate-100 h-full w-full flex flex-col">
		...
		<div class="mt-6 flex items-center bg-primary-light rounded-md text-sm py-2.5 px-2 gap-x-3">
			...
			<input
				placeholder="Search"
				type="search"
				class="grow bg-transparent outline-none placeholder:text-slate-300"
			/>
		</div>
				...
				<HeadlessDisclosure as="div">
					...
					>
						...
						<span v-if="n.items && n.items.length" class="ml-auto">
							<ChevronDown
								class="stroke-white stroke-2 w-3 h-3 ui-open:rotate-180 transition-all duration-300"
							/>
						</span>
					</HeadlessDisclosureButton>

					<HeadlessDisclosurePanel
						class="flex flex-col px-3 text-sm origin-top py-1"
						v-if="n.items && n.items.length"
					>
						<template v-for="(child, k) in n.items" :key="`child-${i}-${k}`">
							...
							>
								<span
									class="w-2 h-2 rounded-full bg-white scale-0 group-hover:scale-100 transition duration-300"
								></span>
								<span>
									{{ child.title }}
								</span>
